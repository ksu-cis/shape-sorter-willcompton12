...
namespace ShapeSorter
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            IEnumerable<Circle> filteredCircles = circles.Where(circle => circle.Area < 20);

            foreach (Circle circle in filteredCircles)
            {
                ///filtered radius less than 20
                Console.WriteLine($"Circle with radius {circle.Radius} and area {circle.Area}");
            }
            Console.WriteLine("-----------------");


            foreach(Circle circle in shapes.OfType<Circle>().Where(c => c.Area > 20))
            {
                Console.WriteLine($"Circle with radius under 20 {circle.Area}");
            }
            Console.WriteLine("-----------------");
